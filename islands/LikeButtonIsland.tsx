...
import Icon from "deco-sites/alan-deco-camp/components/ui/Icon.tsx";
import { invoke } from "deco-sites/alan-deco-camp/runtime.ts";
import { total } from "deco-sites/alan-deco-camp/sdk/useTotalLikes.ts";
...
function LikeButtonIsland({ productID }: LikeButtonIslandProps) {
  ...
  useEffect(() => {
    const updateTotals = async () => {
      const totalLikes = await invoke["deco-sites/alan-deco-camp"].loaders
        .totalLikesLoader();
      const totalLikesProduct = await invoke["deco-sites/alan-deco-camp"]
        .loaders
  ...
  const handleToggleLike = async (e: MouseEvent) => {
    e.preventDefault();
    const result = await invoke["deco-sites/alan-deco-camp"].actions
